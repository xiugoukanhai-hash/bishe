...
export default {
  ...
  methods: {
    init(id, type) {
      ...
      if (crossObj && (crossTable === 'huiyuanruzhu' || type === 'cross')) {
    ...
    onSubmit() {
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          this.$http({
            url: `huiyuantuifang/${!this.ruleForm.id ? 'save' : 'update'}`,
            method: 'post',
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huiyuantuifangCrossAddOrUpdateFlag = false;
                  this.parent.search();
                }
              });
            } else {

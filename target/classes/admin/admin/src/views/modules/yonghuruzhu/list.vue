  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad">
          <el-form-item>
            <el-button v-if="isAuth('yonghuruzhu','新增')" type="success" icon="el-icon-plus" @click="addOrUpdateHandler()">新增</el-button>
            <el-button v-if="isAuth('yonghuruzhu','删除')" :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete" @click="deleteHandler()">批量删除</el-button>
          </el-form-item>
        </el-row>
      </el-form>
      <div class="table-content">
        <el-table class="tables" size="medium" border stripe v-if="isAuth('yonghuruzhu','查看')" :data="dataList" v-loading="dataListLoading" @selection-change="selectionChangeHandler">
          ...
          <el-table-column width="300" header-align="center" align="center" label="操作">
            <template slot-scope="scope">
              <el-button v-if="isAuth('yonghuruzhu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
              <el-button v-if="isAuth('yonghuruzhu','修改') && scope.row.kefangzhuangtai == '已入住'" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
              <el-button v-if="isAuth('yonghuruzhu','用户退房') && scope.row.kefangzhuangtai == '已入住'" type="warning" icon="el-icon-s-home" size="mini" @click="tuifangHandler(scope.row)">退房</el-button>
              <span v-if="scope.row.kefangzhuangtai == '已退房'" style="color: #909399; margin-right: 10px;">已退房</span>
              <el-button v-if="isAuth('yonghuruzhu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
export default {
  ...
  methods: {
    ...
    addOrUpdateHandler(id, type) {
      ...
      this.yonghutuifangCrossAddOrUpdateFlag = false;
      if(type != 'info') {
        type = 'else';
      }
      this.$nextTick(() => {
    ...
    tuifangHandler(row) {
      this.$http({
        url: `yonghutuifang/calcFee/${row.id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
          ...
          message += `总计费用：￥${feeInfo.totalFee || 0}`;
          if (feeInfo.overTimeHours > 0) {
            message += `\n（超时${feeInfo.overTimeHours}小时）`;
          }
          this.$confirm(message, "退房结算确认", {
            ...
            this.$http({
              url: `yonghutuifang/checkOut/${row.id}`,
              method: "post"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.$message({

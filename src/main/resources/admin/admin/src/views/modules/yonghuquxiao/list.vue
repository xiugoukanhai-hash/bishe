  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad">
          <el-form-item>
            <el-button v-if="isAuth('yonghuquxiao','删除')" :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete" @click="deleteHandler()">批量删除</el-button>
          </el-form-item>
        </el-row>
      </el-form>
      <div class="table-content">
        <el-table class="tables" size="medium" border stripe v-if="isAuth('yonghuquxiao','查看')" :data="dataList" v-loading="dataListLoading" @selection-change="selectionChangeHandler">
          ...
          <el-table-column prop="ispay" header-align="center" align="center" label="退款状态">
            <template slot-scope="scope">
              ...
              <el-button v-if="scope.row.sfsh=='已通过' && scope.row.ispay!='已退款' && isAuth('yonghuquxiao','支付')" type="text" icon="el-icon-edit" size="small" @click="refundHandler(scope.row)">退款</el-button>
            </template>
          </el-table-column>
          <el-table-column width="200" header-align="center" align="center" label="操作">
            <template slot-scope="scope">
              <el-button v-if="isAuth('yonghuquxiao','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
              <el-button v-if="isAuth('yonghuquxiao','审核') && scope.row.sfsh=='待审核'" type="primary" icon="el-icon-edit" size="mini" @click="shHandler(scope.row)">审核</el-button>
              <el-button v-if="isAuth('yonghuquxiao','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
  ...
  methods: {
    ...
    refundHandler(row) {
      this.$confirm(`确定对订单 ${row.dingdanbianhao} 进行退款操作?`, "退款确认", {
        ...
        this.$http({
          url: `yonghuquxiao/refund/${row.id}`,
          method: "post"
        }).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message({
